...
HeteroInfer-Lab 是一个面向资源受限边缘环境的大模型推理系统研究项目，关注 LLM（Large Language Model，大语言模型）在单卡 GPU（Graphics Processing Unit，图形处理器）、边缘服务器、小型工作站、FPGA（Field Programmable Gate Array，现场可编程门阵列）、NPU（Neural Processing Unit，神经网络处理器）等平台上的部署边界、性能瓶颈与异构优化路径。
