...
使用 Google Gemma 小模型（优先选择 Gemma 4 E2B），在 Colab GPU 环境下建立基础 profiling 框架。
...
在完成基础 profiling 后，进一步深入 KV Cache。
...
完成第一版实验框架后，再逐步进入 KV Cache 深入分析与异构优化阶段。
