...
* CUDA peak memory overhead can be explained as non-KV runtime / allocator / temporary tensor overhead；
* `experiment_log` records the measurement correction process。

只有在上述条件满足后，才开始 Stage 2：KV Cache System Analysis。

---

# Stage 2：KV Cache System Analysis

在完成 Stage 1 measurement calibration 并满足退出条件后，进一步深入 KV Cache。
