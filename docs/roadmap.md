...
Stage 1 是 measurement 与 baseline characterization 阶段。当前拆分为 Stage 1A 和 Stage 1B。

### Stage 1A：Experiment 001A / PKV Modular Baseline

#### 阶段目标

建立可信 baseline（基线），完成 KV Cache 测量协议校准。
...
- tokens/s 吞吐率测量
- CUDA peak memory 系统级显存压力观测
- `past_key_values` payload 统计
- theoretical KV cache 与 PKV payload 对齐
- prompt length / generation length 控制变量分析

#### 当前状态

Stage 1A 已完成。最终数据以模块化 PKV 结果为准：
...
Stage 1A 的核心结论是：`past_key_values` payload 可作为 pure KV cache 的直接测量方式；CUDA peak memory 不再作为 pure KV cache 指标，而是作为 system-level memory pressure 指标。

### Stage 1B：Experiment 001B / Gemma Model-Scale Stress Baseline

#### 阶段目标

在 Stage 1A 已校准的测量协议基础上，补充 Gemma family model-scale stress baseline，用于观察 Tesla T4 FP16 环境下的模型规模部署边界。

#### 当前状态

Stage 1B 正在进行。当前 v2.1 结果记录在：
...
- Gemma 2 2B IT FP16 可以完成 T4 单 batch 测试矩阵，结果与 Stage 1A 2B baseline 高度一致。
- Gemma 2 9B IT FP16 在模型加载阶段发生 CUDA OOM，是 T4 FP16 部署边界的直接证据。
- 是否补充 Gemma 4 探测尚未最终确认，因此 Stage 1 不应写作完全关闭。

#### 与 Stage 2 的关系

Stage 1B 为 Stage 2 的 deployment boundary analysis 提供容量边界证据。Stage 2 仍然是 next stage，应在 Stage 1B 范围确认后正式展开。
...
## 完成 Stage 1B 收尾并准备进入 Stage 2

包括：

- 固化 Stage 1A baseline profiling benchmark
- 完成 PKV measurement protocol 文档化
- 整理 Experiment 001B model-scale stress baseline 结果
- 明确 Gemma 2 2B / 9B 在 T4 FP16 下的部署边界观察
- 决定是否补充 Gemma 4 Stage 1B 探测
- 在 Stage 1B 范围确认后，建立 Stage 2 的标准化实验入口

后续所有 CUDA、FPGA、NPU 与 MLIR 工作，均建立在可信 baseline、部署边界观察与明确 bottleneck 的基础之上。
